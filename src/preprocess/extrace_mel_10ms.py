...
def main():
    parser = argparse.ArgumentParser(
        description="Extract Mel-spectrogram features from audio files"
    )
    parser.add_argument(
        "--input_dir", type=str, required=True, help="Input audio directory"
    )
    parser.add_argument(
        "--output_dir", type=str, required=True, help="Output mel feature directory"
    )
    parser.add_argument(
        "--file_list",
        type=str,
        default=None,
        help="Optional file list (one relative path per line)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--hop_length", type=int, default=160, help="Hop length (10ms at 16kHz)"
    )
