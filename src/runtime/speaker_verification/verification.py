...
def init_model(model_name, checkpoint=None, num_class: int = 0):
    ...
    if model_name == 'unispeech_sat':
        model = ECAPA_TDNN_SMALL(feat_dim=1024, feat_type='unispeech_sat', num_class=num_class)
    elif model_name == 'wavlm_base_plus':
        model = ECAPA_TDNN_SMALL(feat_dim=768, feat_type='wavlm_base_plus', num_class=num_class)
    elif model_name == 'wavlm_large':
        model = ECAPA_TDNN_SMALL(feat_dim=1024, feat_type='wavlm_large', num_class=num_class)
    elif model_name == 'hubert_large':
        model = ECAPA_TDNN_SMALL(feat_dim=1024, feat_type='hubert_large_ll60k', num_class=num_class)
    elif model_name == 'wav2vec2_xlsr':
        model = ECAPA_TDNN_SMALL(feat_dim=1024, feat_type='wav2vec2_xlsr', num_class=num_class)
    else:
...
if __name__ == "__main__":
    ...
    ckpt_path = os.path.join(project_root, 'assets', 'wavLM', 'wavlm_large_finetune.pth')
