...
class Trainer:
    ...
    def validate(self, step):
        ...
        vocos = Vocos.load_selfckpt("/vc_10ms/version_0").to(self.device)
        ...
        sv_model.eval()
        sv_model.to(self.device)
        ssmi_dict = {}

        # cer
        asr_model = load_zh_model(str(self.device))
        wer_dict = {}
        utt2txt = {}
        text_all = ""
        ...
        time_points = torch.linspace(1.0, 0.0, steps + 1, device=self.device)
        ...
        with torch.no_grad():
            for i, spk in enumerate(spks):
                ...
                prompt_mel = torch.from_numpy(prompt_mel).to(self.device)
                ...
                for bn_path in bns:
                    bn = torch.from_numpy(np.load(bn_path)).to(self.device)
                    bn = bn.unsqueeze(0)
                    bn = bn.transpose(1, 2)
